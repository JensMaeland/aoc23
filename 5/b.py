...
def getSeedList(startIndex, rangeInput):
    seeds = []
    for i in range(startIndex, startIndex + rangeInput):
        seeds.append(Seed(i))
    return seeds


print(getSeedList(79, 14))
...
def read_input(input):
    ...
    for line in input:
        ...
        # Update which map should be written to
            ...
            for i in range(0, len(seedsInLine), 2):
                startIndex = int(seedsInLine[i])
                rangeInput = int(seedsInLine[i + 1])
                seedList = getSeedList(startIndex, rangeInput)
                seeds.extend(seedList)
            continue
